...
function diffSchemas(previousSchema: any, nextSchema: any) {
  const changes: any[] = [];
  ...
  const prevParams = new Set((previousSchema.parameters || []).map((p: any) => p.name));
  const nextParams = new Set((nextSchema.parameters || []).map((p: any) => p.name));
